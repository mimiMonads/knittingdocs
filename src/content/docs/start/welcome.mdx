...
Yet, in general application code, workers are still surprisingly rare. Not because parallelism isn’t useful, but because the default solutions drifted elsewhere.

Promises made I/O concurrency less painful. When servers got hot, it often felt
...
- **Event-loop friendly:** runtimes cooperate with the event loop: no blocking,
  no forced `await`, just predictable behavior.
...
- **Math and simulation:** [Big prime](/examples/maths/big_prime), [Monte Carlo](/examples/maths/monte_pi), [Physics loop](/examples/maths/physics_loop),
...
(`yes, you Spectre`). Until we know it is safe, Knitting stays server-side by
design.
...
**Remote imports are currently supported (with caution)**  
Knitting can load task modules from remote sources, but this expands trust
boundaries and can reduce reproducibility across environments.

Because of security and supply-chain concerns, remote import support may be
restricted or removed in future releases.

**Not a distributed system**  
Knitting is not a cluster manager, or a distributed execution framework. It operates within a single runtime instance, across threads, using shared memory. If you need cross-machine distribution, Knitting is meant to complement that layer, not replace it.
