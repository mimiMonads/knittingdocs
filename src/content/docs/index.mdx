...
Knitting lets you run tasks across worker threads as if they were normal function calls. Under the hood it uses shared memory to keep coordination fast, and it works on Node.js, Deno, and Bun.
Benchmarks here focus on coordination/transfer cost, not guaranteed whole-app speedup.
Curious how it actually works? Read the architecture section below, then check the benchmarks to see it in action.
