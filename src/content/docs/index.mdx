...
- **Deterministic call boundary:** promise inputs are resolved on the host; workers only receive plain values.  
  If an input promise rejects, the task is not executed. ([Defining tasks](/guides/defining-tasks/))
- **Clear payload rules:** supported types are explicit; functions are not payloads. ([Supported payloads](/guides/payloads/))
