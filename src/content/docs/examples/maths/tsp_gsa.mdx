...
# The NP Problem: Traveling Salesman Problem (TSP)
...
* **High-throughput compute**: Knitting’s worker scheduling + batching (`fastCall` + `send()`)
...
* each agent stores a real-valued vector 
...
* different `runSeed` (different search trajectory)
...
* enqueue many calls via `fastCall`
* then call `send()` once
