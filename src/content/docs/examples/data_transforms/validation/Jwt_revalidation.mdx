...
2. `utils.ts` isolates token validation + renewal logic and exports tasks.
3. `bench_jwt_revalidation.ts` runs a host-vs-worker benchmark with `mitata`.
...
- It isolates core JWT logic in one reusable utility module.
- It keeps the benchmark simple and reproducible.
