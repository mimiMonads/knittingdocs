...
export const component = getCode("data_transforms/salt_hashing/salt_hashing.ts");
...
2. `salt_hashing.ts` isolates hashing, verification, and packet fast-path functions.
3. `bench_salt_hashing.ts` runs a quick benchmark with fixed settings (no flags).
...
Bun
```bash
bun src/salt_knitting.ts
```
Deno
```bash
deno run -A src/salt_knitting.ts
```
Node
```bash
npx tsx src/salt_knitting.ts
```
:::
...
Bun
```bash
bun src/bench_salt_hashing.ts
```
Node
```bash
npx tsx src/bench_salt_hashing.ts
```
:::
...
<Tabs>
  ...
  <TabItem label="salt_hashing.ts">
    <Code code={component} lang="ts" title={"salt_hashing.ts"} />
  </TabItem>
</Tabs>
