...
Bun
```bash
bun add hono @hono/node-server react react-dom zod
```
Bun (Knitting mode only)
```bash
bun add @vixeny/knitting
```
Node
```bash
npm i hono @hono/node-server react react-dom zod
```
:::
...
Node
```bash
JWT_SECRET="replace-me" npx tsx src/hono_knitting.ts
```
:::
...
<Tabs>
  <TabItem label="hono_knitting.ts (server)">
    <Code code={server} lang="ts" title={"hono_knitting.ts"} />
  </TabItem>
  <TabItem label="hono_componets_ssr.tsx (SSR task)">
    <Code code={renderSsrPage} lang="tsx" title={"hono_componets_ssr.tsx"} />
  </TabItem>
  <TabItem label="hono_components_jwt.ts (JWT task)">
