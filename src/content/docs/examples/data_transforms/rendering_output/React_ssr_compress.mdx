...
:::info
<RuntimeRunTabs
  bunCommand="bun src/react_ssr_compress_knitting.ts"
  denoCommand="deno run -A src/react_ssr_compress_knitting.ts"
  nodeCommand="npx tsx src/react_ssr_compress_knitting.ts"
/>
:::
...
2. The host path renders HTML via `renderUserCardHost` (string output), then compresses with Brotli.
