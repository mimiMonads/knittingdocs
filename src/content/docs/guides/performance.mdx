...
- <Badge text="Meh" variant="caution" />: watch frequency
...
Values fit into the call header: tiny encode/decode cost. Best for primitives and sentinel values.

**2) Static payload**  
Reuses part of the Header plus a small buffer.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>(1)</code></td>
      <td><Badge text="Meh" variant="caution" /></td>
    </tr>
    <tr>
...
Static payloads reuse a cached representation. Dynamic payloads pay allocation/bookkeeping cost per call.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>Uint8Array</code>: static</td>
      <td><code>(1)</code></td>
      <td><Badge text="Best" variant="note" /></td>
    </tr>
    <tr>
      <td><code>Uint8Array</code>: dynamic</td>
      <td><code>(1)</code></td>
      <td><Badge text="Fast" variant="success" /></td>
    </tr>
    <tr>
...
## Practical advice (the short version)

- If you’re calling tasks **a lot**, prefer:
  - primitives,
  - typed arrays,
  - or static payloads where repetition exists.
- If you’re moving collections (<code>Map</code>, <code>Set</code>) or heavy structured data, batch aggressively.
- If performance is borderline, push more work per call (fewer round trips).
