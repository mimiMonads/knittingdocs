...
- `fastCall.<task>(args)` like `call`, but optimized for tight loops.
- `send()` kick the dispatcher loop (useful after enqueueing a batch).
- `shutdown()` stop all workers.

`call.*()` always returns a promise. Inputs may also be promises; they are
resolved on the host before dispatch (rejections propagate to the caller).
