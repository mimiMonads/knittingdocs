...
- `1` (default): inline lane is immediately eligible (same behavior as before).
- Higher values: keep the host lane out of scheduling at light concurrency, then
  let it join once load bursts.
- If `inliner` is not enabled, this option has no effect.

:::note
Migration: `host.dispatchThreshold` was removed.
Use `inliner.dispatchThreshold` instead.
:::
...
Revisit tuning as rebalance strategies land.
