...
| `signal.now()` <Badge text="next release" variant="caution" /> | A monotonic millisecond clock, for measuring elapsed time inside the task. |

:::caution[`signal.now()` is not out yet]
On the current release (`0.1.56`) the toolkit is `{ hasAborted }` only, and
`signal.now()` is `undefined`. Everything else in this section works today.
:::
