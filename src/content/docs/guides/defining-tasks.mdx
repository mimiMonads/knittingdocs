...
- `call` returns `Promise<Awaited<ReturnType<TaskFn>>>`, so consumers get the
  final resolved type.
...
const addOne = task<Promise<number> | number, number>({
  f: (n) => n + 1, // n is already awaited here
});

const pool = createPool({ threads: 1 })({ addOne });

// Promise input is accepted.
