...
- Abort signals on inline tasks use a static toolkit where `hasAborted()` always returns `false` (inline tasks can't be individually cancelled since they share the host thread). From the next release the toolkit also carries `now()`, which behaves normally and reads the host's clock.
