---
title: Benchmarks
description: Overall benchmark overview across Node, Deno, and Bun, plus a Tokio comparison for primitive and small-payload benchmark shapes.
sidebar:
  order: 1
---
...
This page is the high-level benchmark summary across Node, Deno, and Bun, with a Tokio close-up for the small-payload fast path.
For runtime-specific detail, see the dedicated pages for Node, Deno, Bun, and Tokio.

These benchmarks quantify communication overhead and scaling behavior in specific benchmark shapes.
They are not a direct prediction of end-to-end application speedup.
...
For primitive calls and low binary payloads, Knitting stays in the same microsecond tier as Tokio in these benchmark shapes.
Read that as handoff cost: wakeups/signaling plus copying or cloning very small payloads, not a claim that every payload shape matches Tokio.
That matters because it shows the shared-memory fast path is not just "good for JavaScript" — it can stay competitive with a Rust baseline when coordination dominates.
...
From the `1048576 B` row (`avg`) in each runtime's `*_call-growth` result, one-way transfer throughput is:
...
- Bun is clearly fastest at 1 MiB in this benchmark shape, for both text and binary payloads.
- Deno's 1 MiB string path is notably stronger than Node, while binary (`Uint8Array`) is close between Deno and Node.
